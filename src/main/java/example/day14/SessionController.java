...
public class SessionController {
    ...
    public ResponseEntity<?> test1(@RequestParam String data, HttpServletRequest request){
        ...
        System.out.println( request.getRemoteAddr() ); // 요청한 클라이언트(사용자) IP 주소 얻기( 로그/추적 ) -> 지도 쓰는 사람 ip로 위치 찾기 가능함.
        System.out.println( request.getHeader("User-Agent")); // 요청한 클라이언트 브라우저 정보
        System.out.println( request.getSession() ); //  요청한 클라이언틔 세션객체 정보 * 각 브라우저마다 할당
        // 2) 세션 객체
        HttpSession session = request.getSession();
        System.out.println( session.getId() ); // 세션의 식별번호 반환, 브라우저/디바이스마다 할당
    ...
    // [2] 세션객체 내 값 호출 = 로그인 한 회원정보(마이페이지)
    @GetMapping("")
    public ResponseEntity<?> test2( HttpServletRequest request ){
        System.out.println( request.getHeader("User-Agent" ) );
        // 1) 세션객체 반환
        HttpSession session = request.getSession();
        // 2) 새션객체 내 특정한 속성 반환 * 모든 값은 Object 반환된다. *
    ...
    public ResponseEntity<?> test3( HttpSession session ){ // 매개변수로 HttpSession 받는다.
        // 방법 1) 세션 전체 초기화
//        session.invalidate(); // 세션객체 내 모든속성 제거
        // 방법 2) 특정 속성 초기화
...
/*
    톰캣 세션
        ...
        3. 사용처: 1) *로그인*상태 ( 세션 - 서버내부 ) / ( JWT - 서버외부 = 클라이언트 ) 2) 장바구니 3) 실시간처리 등등
        4. 특징: 브라우저(크롬/엣지/사파리/TV 등)마다 *별도의 세션 객체* 할당된다.
    ...
    HttpSession
        1. 세션: 메모리가 저장되는 구역(세션 종류가 많음)의 일부
        2. 톰캣 세션: HTTP 객체 내 제공받는 메모리 구역
        3. 주요 메소드
            1) .setAttribute( "key" , value )       : 세션객체 내 속성명과 속성값 저장, 주로 로그인 상태/정보
            2) .getAttribute( "key" )               : 세션객체 내 속성명 이용한 속성값 호출, MAP과 유사 *Object로 반환*
 */
